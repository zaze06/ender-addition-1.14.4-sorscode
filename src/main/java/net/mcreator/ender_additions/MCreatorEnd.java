...
public class MCreatorEnd extends Elementsender_additions.ModElement {
	...
	private static LiteralArgumentBuilder<CommandSource> customCommand() {
		return LiteralArgumentBuilder.<CommandSource> literal("end")
				.then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(MCreatorEnd::execute))
				.executes(MCreatorEnd::execute);
	}
	...
	private static int execute(CommandContext<CommandSource> ctx) {
		...
		if (entity != null) {
			...
			});
			{
				...
				MCreatorEndCommandExecuted.executeProcedure($_dependencies);
			}
		}
		return 0;
	}
